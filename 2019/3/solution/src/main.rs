...
fn main() -> Result<(), Error> {
    ...
    // Part 1
    writeln!(io::stdout(), "Min intersection distance: {:?}", find_min_intersection_distance(&wire_1, &wire_2).unwrap())?;

    // Part 2
    writeln!(io::stdout(), "Min steps intersection: {:?}", find_min_steps_intersection(&wire_1, &wire_2).unwrap())?;
...
fn find_min_steps_intersection(wire_1: &Vec<Position>, wire_2: &Vec<Position>) -> Option<u32> {
    ...
    for intersection in get_intersections(&wire_1, &wire_2) {
        if let (Some(steps_w1), Some(steps_w2)) = (steps_to_position(intersection, &wire_1), steps_to_position(intersection, &wire_2)) {
